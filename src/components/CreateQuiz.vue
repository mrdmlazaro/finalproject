    ...
    <div>
        <v-container class="container" grid-list-md>
            <v-layout wrap>
                ...
                <v-flex xs8>
                    <v-card max-width="1020" class="mx-auto">
                        <v-card-title>
                            <h2>Create Quiz</h2>
                        </v-card-title>
                        ...
                        <v-card-text>
                            ...
                            <v-text-field v-model="quizTitle" label="Quiz Title" required></v-text-field>

                            <v-textarea v-model="quizDescription" label="Description" hint="Type in a short description of the quiz">
                            </v-textarea>

                            <v-btn @click="addNewQuestion = true">Add Question</v-btn>

                            <v-select v-if="addNewQuestion == true" v-model="selectedQuestionType"
                                :items="questionTypesDropdown" item-value="value" item-text="text"
                                label="Choose one type" outlined>
                            </v-select>

                            <div v-if="addNewQuestion == true" class="card-body">
                                <v-form v-on:submit.prevent="addQuestion">

                                    <v-text-field v-model="question.type" label="Type" required disabled></v-text-field>

                                    <v-text-field v-model="question.text" label="Question" required></v-text-field>

                                    <v-text-field v-if="selectedQuestionType == 'mc'" v-model="question.answers"
                                        label="Choices"></v-text-field>
                ...
                <v-flex xs4>
                    <v-form v-if="newQuestionAdded == true" v-on:submit.prevent="addQuiz">
                    ...
                    <div v-else>
                        <v-card>
                            <v-card-title>
                                It's empty here, no questions created yet.
                            </v-card-title>
                        </v-card>
                    </div>
                </v-flex>
